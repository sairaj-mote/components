...
customElements.define('sm-button', class extends HTMLElement {
    constructor() {
        super();
        this.attachShadow({
            mode: 'open'
...
`;
customElements.define('sm-input', class extends HTMLElement {
    constructor() {
        super();
...
`;
customElements.define('sm-textarea', class extends HTMLElement {
    constructor() {
        super();
        this.attachShadow({
            mode: 'open'
...
customElements.define('sm-select', class extends HTMLElement {
    ...
    connectedCallback() {
        ...
        let slot = this.shadowRoot.querySelector('.options slot'), selection = this.shadowRoot.querySelector('.selection'), previousOption;
        this.open = false;
        this.slideDown = [{
...
customElements.define('sm-strip-select', class extends HTMLElement {
    ...
    connectedCallback() {
        let previousOption, slot = this.shadowRoot.querySelector('slot');
    ...
    constructor() {
        ...
        this.show = (pinned, popupStack) => {
            ...
            if (this.popupStack && !this.hasAttribute('open')) {
                ...
                this.dispatchEvent(new CustomEvent("popupopened", {
                    bubbles: true,
                    detail: {
                        popup: this,
...
customElements.define('sm-carousel', class extends HTMLElement {
    ...
    connectedCallback() {
        ...
        let firstVisible = false, lastVisible = false;
...
customElements.define('sm-notifications', class extends HTMLElement {
    constructor() {
        ...
        this.push = (messageBody, type, pinned) => {
            let notification = document.createElement('div'), composition = ``;
    ...
    <svg class="notification-icon icon error-icon" viewBox="0 0 64 64">
        ...
        this.removeNotification = (notification, toLeft) => {
            if (!this.offset)
                this.offset = 0;
            if (toLeft)
                notification.animate([{
...
customElements.define('sm-menu', class extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.optionList = this.shadowRoot.querySelector('.options');
        let slot = this.shadowRoot.querySelector('.options slot'), menu = this.shadowRoot.querySelector('.menu');
...
customElements.define('sm-tab-header', class extends HTMLElement {
    ...
    connectedCallback() {
        if (!this.hasAttribute('target') || this.getAttribute('target').value === '')
            return;
        this.prevTab;
        this.allTabs;
        this.activeTab;
    ...
    connectedCallback() {
        ...
        document.addEventListener('switchtab', e => {
            if (e.detail.target !== this.id)
                return;
            if (this.prevPanel) {
                let targetBody = this.allPanels[e.detail.rank], currentBody = this.prevPanel;
